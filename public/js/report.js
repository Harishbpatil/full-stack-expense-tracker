const axiosReportInstance = axios.create({
  baseURL: "http://100.26.137.165:4000/report",
  headers: {
...
document
  ...
  .addEventListener("submit", async (e) => {
    ...
    const month =
      e.target["year-picker"].value + "-" + e.target["month-picker"].value;
    console.log(month);
    try {
      ...
      let total = 0;
      tbody.innerHTML = ``;
      document.querySelector("#monthly h3 span").textContent =
        e.target["month-picker"].options[
          e.target["month-picker"].selectedIndex
        ].text;
      res.data.forEach((elem) => {
        console.log(elem);
        const tr = document.createElement("tr");
